...
function es_exportAllStreamsPDF() {
  ...
  generated.forEach((cls, pageIdx) => {
    ...
    // Dark background
    doc.setFillColor(15,17,23); doc.rect(0,0,pw,ph,'F');

    // School name + class title
    doc.setFontSize(11); doc.setFont(undefined,'bold'); doc.setTextColor(232,234,240);
    doc.text(`${es_state.school.name||'School'}  —  ${cls.grade}${cls.stream?' '+cls.stream:''}  —  Weekly Timetable`, margin, 12);
    doc.setFontSize(7); doc.setFont(undefined,'normal'); doc.setTextColor(136,146,170);
    doc.text(`Generated: ${new Date().toLocaleDateString()}  •  Page ${pageIdx+1} of ${generated.length}`, margin, 17);
    ...
    for (let i = 0; i < cols.length; i++) {
      ...
      if (col.type==='day') {
        doc.setFillColor(30,34,48); doc.rect(x0,y,cw,rowH,'F');
        doc.setTextColor(136,146,170); doc.setFontSize(7); doc.text('Day',x0+2,y+11);
      } else if (col.type==='break') {
        const isL=/lunch/i.test(col.breakObj.label);
        doc.setFillColor(isL?209:254,isL?250:243,isL?229:199); doc.rect(x0,y,cw,rowH,'F');
        doc.setTextColor(isL?6:146,isL?95:64,isL?70:14); doc.setFontSize(5);
        doc.text((col.breakObj.label.length>6?col.breakObj.label.substring(0,5)+'.':col.breakObj.label),x0+1,y+7);
        doc.text(`${col.breakObj.duration}m`,x0+1,y+13);
      } else {
        doc.setFillColor(30,34,48); doc.rect(x0,y,cw,rowH,'F');
        doc.setTextColor(136,146,170); doc.setFontSize(7); doc.text(`P${col.periodIndex+1}`,x0+1,y+7);
        doc.setFontSize(5); doc.text(es_getPeriodTime(col.periodIndex),x0+1,y+13);
      }
    }
    y += rowH;
    ...
    for (let d = 0; d < days; d++) {
      ...
      for (let i = 0; i < cols.length; i++) {
        ...
        if (col.type==='day') {
          doc.setFillColor(21,25,35); doc.rect(x0,ry,cw,rowH,'F');
          doc.setTextColor(180,195,220); doc.setFontSize(7); doc.text(ES_DAY_NAMES[d],x0+2,ry+11);
        } else if (col.type==='break') {
          const isL=/lunch/i.test(col.breakObj.label);
          doc.setFillColor(isL?209:254,isL?250:243,isL?229:199); doc.rect(x0,ry,cw,rowH,'F');
          doc.setTextColor(isL?6:146,isL?95:64,isL?70:14); doc.setFontSize(9);
          doc.text(isL?'🍽':'☕',x0+2,ry+12);
        } else {
          ...
          if (sub) {
            ...
            doc.rect(x0,ry,cw,rowH,'F');
            doc.setTextColor(255,255,255); doc.setFontSize(7);
            doc.text(sub.name.length>13?sub.name.substring(0,12)+'…':sub.name,x0+1,ry+7);
            const tch=slot.teacherId?es_state.teachers.find(t=>t.id===slot.teacherId):null;
            if (tch) { doc.setFontSize(5.5); doc.text(tch.name.length>16?tch.name.substring(0,15)+'…':tch.name,x0+1,ry+13); }
            else     { doc.setFontSize(5); doc.setTextColor(255,200,150); doc.text('⚠ no teacher',x0+1,ry+13); }
          } else { doc.setFillColor(22,25,34); doc.rect(x0,ry,cw,rowH,'F'); }
        }
      }
    }
  });

  const schoolSlug = (es_state.school.name||'school').replace(/\s+/g,'_');
  doc.save(`all_timetables_${schoolSlug}.pdf`);
  es_toast(`📄 PDF exported — ${generated.length} stream(s)!`,'success');
}

/* =====================================================
   PRINT — All Streams (browser print dialog)
===================================================== */
function es_printAllStreams() {
  ...
  const subColors = {};
  es_state.subjects.forEach(s => { subColors[s.id] = s.color || '#4f7cff'; });

  const blocks = generated.map(cls => {
    ...
    const hdrCells = cols.map(col => {
      ...
      if (col.type==='break') {
        const isL=/lunch/i.test(col.breakObj.label);
        return `<th class="bc ${isL?'lc':''}">${isL?'🍽':'☕'}<br><small>${col.breakObj.label}</small></th>`;
      }
      return `<th class="pc">P${col.periodIndex+1}<br><small>${es_getPeriodTime(col.periodIndex)}</small></th>`;
    }).join('');

    const rows = Array.from({length:days},(_,d)=>{
      const cells = cols.map(col => {
        if (col.type==='day') return `<td class="dn">${dayNames[d]}</td>`;
        if (col.type==='break') { const isL=/lunch/i.test(col.breakObj.label); return `<td class="bk ${isL?'lk':''}">${isL?'🍽':'☕'}</td>`; }
    ...
    return `<div class="pg">
      <div class="hd">${es_state.school.name||'School'} — ${cls.grade}${cls.stream?' '+cls.stream:''} — Weekly Timetable</div>
      <div class="dt">Generated: ${new Date().toLocaleDateString()}</div>
  ...
  win.document.write(`<!DOCTYPE html><html><head><title>All Timetables — ${es_state.school.name||'School'}</title>
  <style>
    *{box-sizing:border-box;margin:0;padding:0}
    body{font-family:Arial,sans-serif;font-size:9pt;background:#fff;color:#111}
    .pg{page-break-after:always;padding:10mm 10mm 6mm;min-height:190mm}
    .pg:last-child{page-break-after:avoid}
    .hd{font-size:13pt;font-weight:bold;margin-bottom:2mm}
    .dt{font-size:7pt;color:#666;margin-bottom:3mm}
    table{width:100%;border-collapse:collapse;table-layout:fixed}
    th,td{border:1px solid #ccc;text-align:center;vertical-align:middle;padding:2px 1px;font-size:7.5pt}
    th{background:#1e2230;color:#e8eaf0;font-weight:600}
    .dc,.dn{width:28px;font-weight:bold}
    .bc,.bk{width:18px;background:#fef3c7!important;color:#92400e;font-size:6pt}
    .lc,.lk{background:#fef9c3!important;color:#713f12}
    .em{background:#f1f5f9}
    .ls{color:#fff;padding:2px}
    .sn{display:block;font-weight:700;font-size:7.5pt}
    .tn{display:block;font-size:6pt;opacity:.9}
    .dn{background:#15192380;color:#b4c3dc;font-size:7pt}
    .legend{margin:0 10mm 4mm;display:flex;flex-wrap:wrap;gap:4px}
    .leg{display:flex;align-items:center;gap:3px;font-size:7pt}
